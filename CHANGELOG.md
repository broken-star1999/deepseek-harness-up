...
- npm.cmd 统一经过隐藏批处理执行器，参数安全校验覆盖 `%*` 展开场景，修复 Windows `cmd /C` 参数注入风险
- 自定义镜像日志仅记录主机名，不记录完整 URL；应用数据路径拒绝相对路径、父目录跳转和异常环境回退
- Tauri CSP 收紧为本地资源、dsh 本地端口和必要的 GitHub API

### 🐛 修复
- dsh 状态区分未运行、启动中和已就绪，启动超时只清理本工具自己创建的进程树
- 更新、安装、卸载和启停操作统一互斥；运行中或启动中拒绝更新/安装，卸载前等待 3080 释放
- 3080 进程识别要求 Node 可执行路径和精确 `lib/bin.js` 入口参数匹配；netstat 端口按字段精确解析，避免误识别相邻端口
- 端口被非 dsh 程序占用时不再自动结束，环境体检会明确显示占用者和可执行操作
- 设置读改写全程加锁，临时文件落盘后原子替换并保留 `.bak` 恢复
- 前端接入环境体检结果，修复启动态按钮、卸载预览、失败反馈、IPC 和 DOM 引用问题

### 🗑 卸载
- 保留用户明确授权的完整 dsh 卸载：可移除全局包、完整 `DSH_HOME` 用户数据和可选 npx 缓存
- 卸载前展示目标路径，拒绝符号链接、盘符根目录、用户目录祖先、UNC 和不安全路径
- 包卸载成功但用户数据删除失败时返回结构化部分完成结果，不误报为完全成功

### 🔧 构建与质量
- 发布目标收口为单个便携 EXE，关闭 Tauri MSI/NSIS bundle；`dist/` 发布闸门拒绝额外文件
- CI 增加 locked Rust 检查、release 构建、npm 审计、IPC/DOM/HTML 事件属性和单 EXE 契约检查
- 新增批处理注入、dsh 身份、精确端口和隔离完整卸载测试；Rust 单元测试最终 16/16 通过
